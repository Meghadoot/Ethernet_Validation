/***************************************************************************
    Ethernet Speed Test.c
    Author: Meghadoot
    2025-03-03
    
    
    This is a library for the validation of Ethernet Interface.
    This validation test uses iperf to test speed.
...
int main() {
    printf("Measuring Ethernet throughput...\n");
    char command[256];
    snprintf(command, sizeof(command), "iperf -c %s -t 60 > ethernet_performance_test.log", SERVER_IP);
    system(command);

    printf("Ethernet Performance Test completed. Results:\n");
    system("cat ethernet_performance_test.log");
